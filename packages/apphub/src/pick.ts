...
export function tintOf(slug: string): string {
  let n = 0;
  for (let i = 0; i < slug.length; i += 1) n = (n + slug.charCodeAt(i)) % 997;

  return TINTS[n % TINTS.length];
}

export function familyRows(items: FamilyApp[], platform: string): FamilyRow[] {
  const ios = platform === 'ios';
  const urlOf = (a: FamilyApp): string | null => (ios ? a.ios : a.android) ?? null;
