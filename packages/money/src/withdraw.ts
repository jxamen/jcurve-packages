...
export const DEFAULT_CASH: CashRules = { min: 1000, max: 50000, unit: 1000 };

/** 무엇이 잘못됐나 — 화면이 사유별로 다르게 말할 수 있게 코드로 준다 */
export type CashProblem =
  | 'empty'        // 안 적었다
  | 'not_number'   // 숫자가 아니다
  | 'unit'         // 단위가 안 맞는다
  | 'min'          // 너무 적다
 ...
 */
export function checkCash(input: string, balance: number, rules: CashRules = DEFAULT_CASH): CashCheck {
  const raw = String(input ?? '').replace(/[,\s원]/g, '');
  if (raw === '') return { ok: false, problem: 'empty' };
  if (!/^\d+$/.test(raw)) return { ok: false, problem: 'not_number' };

  const amount = Number(raw);
  const unit = Math.max(1, Math.trunc(rules.unit));

  if (amount % unit !== 0) return { ok: false, problem: 'unit' };
  if (amount < rules.min) return { ok: false, problem: 'min' };
  if (amount > rules.max) return { ok: false, problem: 'max' };
  if (amount > balance) return { ok: false, problem: 'balance' };
 ...
 */
export function amountChoices(balance: number, rules: CashRules = DEFAULT_CASH): number[] {
  const unit = Math.max(1, Math.trunc(rules.unit));
  // 가진 것과 한도 중 **작은 쪽**까지. 단위로 내림 — 3,500P 면 3,000원까지다
  const ceiling = Math.floor(Math.min(rules.max, Math.trunc(balance)) / unit) * unit;

  const out: number[] = [];
  for (let v = Math.max(rules.min, unit); v <= ceiling; v += unit) out.push(v);
...
export function accountLooksOk(v: string): boolean {
...
export function phoneLooksOk(v: string): boolean {
  return /^01[016789]\d{7,8}$/.test(cleanPhone(v));
}

/** 신청에 필요한 것이 다 찼나 — 버튼을 켤지 정한다 */
export function cashFormReady(f: {
  name: string;
  bank: string;
  account: string;
  amount: string;
}, balance: number, rules: CashRules = DEFAULT_CASH): boolean {
