...
**`photo.uri` 는 사진 선택기·문서 스캐너가 준 것만 넘긴다.** 패키지는 받은 uri 를 그대로
올린다. 딥링크처럼 바깥에서 uri 를 정할 수 있는 자리에 연결하면 앱 안의 파일이 서버로
올라갈 수 있다. 막는 자리는 패키지가 아니라 **uri 를 받는 쪽**이다 — uri 만으로는
사용자가 고른 사진인지 앱 내부 파일인지 가를 수 없다(둘 다 `file://`).
