...
function defaultEnv() {
    const rn = () => mod(() => require('react-native'));
    const os = () => String(rn()?.Platform?.OS ?? '');
    // 웹 미리보기에서는 SDK 를 쓰지 않는다 — 모듈은 불러와져도 네이티브가 없다
    const native = (load) => () => (os() === 'web' ? null : mod(load));
    return {
        os,
...
const errText = (e) => String(e?.code ?? '') + ' ' + String(e?.message ?? e ?? '');
/** 퍼널 꼬리표로 쓸 짧은 코드 — 영문·숫자만, 20자 */
const shortCode = (e) => String(e?.code ?? e?.message ?? 'err').replace(/[^A-Za-z0-9_]/g, '').slice(0, 20) || 'err';
/** `abandon()` 으로 잊힌 실행이 멈출 때 — 화면은 이미 풀렸으니 취소처럼 조용히 끝낸다 */
const abandoned = () => new AuthError('cancelled', 'abandoned');
...
function createAuth(deps, env = defaultEnv()) {
     ...
     * 불러 보지도 못하고 실패했다.
     */
    function addCandidate(url, ticket, front = false) {
        if (used.has(ticket))
            return false;
        const i = freshCandidates().findIndex((c) => c.ticket === ticket);
        if (i >= 0) {
            if (front && i > 0)
                candidates.unshift(...candidates.splice(i, 1));
            return false;
        }
    ...
    const FLOW_TTL = 10 * 60000;
    const flowAlive = () => !!flow && flow.open && Date.now() - flow.at < FLOW_TTL;
    /** 복귀 주소가 **정확히** 우리 것인가 — 앞부분만 같은 `returnUrl/evil` 이나 `returnUrl/` 는 아니다 */
    const base = (u) => u.split('#')[0].split('?')[0];
    const isReturn = (u) => !!web && base(u) === base(web.returnUrl);
    const capture = (url) => {
        if (typeof url !== 'string' || !isReturn(url))
            return;
        const { ticket } = parseReturn(url);
        if (!ticket || used.has(ticket) || !flowAlive())
            return;
        if (!addCandidate(url, ticket))
            return;
        // 로그인이 도는 중이면 그쪽이 가져간다 — 밖에서 들어왔을 때만 화면에 알린다
        const p = flow.provider;
        if (!current)
            lateListeners.forEach((f) => { try {
                f(p);
            }
            catch { /* 화면 쪽 오류는 삼킨다 */ } });
    };
    if (web) {
        const L = env.linking();
        try {
            L?.addEventListener?.('url', (e) => capture(e?.url));
        }
        catch { /* 없으면 창 결과만 쓴다 */ }
        /*
    ...
    let kakaoInit = null;
    function initKakao() {
        if (isPlaceholder(keys.kakaoNative))
    ...
    function availableProviders() {
        ...
        /*
         ...
         | 괄호를 붙이는 순간 ③ 을 밟는다.
         */
        if (!isPlaceholder(keys.kakaoNative) && env.kakaoUser()?.login)
            list.push('kakao');
        if (!isPlaceholder(keys.googleWeb) && env.google()?.GoogleSignin)
            list.push('google');
        // 애플 로그인은 iOS 에서만 — 소셜 로그인만 제공하는 앱은 심사지침 4.8 로 필수다
        if (env.os() === 'ios' && env.apple())
            list.push('apple');
        return list;
    }
    ...
    async function kakaoToken() {
        ...
        if (!canAsk)
            track('login_native_fallback', { provider: 'kakao', code: 'cannot_tell' });
        const talk = canAsk ? await kakaoTalkAvailable() : false;
        if (canAsk && !talk)
            track('login_native_fallback', { provider: 'kakao', code: 'no_talk' });
        try {
    ...
    function retryable(e) {
        ...
        if (Number.isFinite(status) && status >= 400)
            return false;
        if (/network|request failed|failed to fetch|abort|timed? ?out|offline|socket/i.test(msg + ' ' + kind))
            return true;
    ...
    async function withRetry(fn) {
        ...
        for (let i = 0;; i++) {
            try {
                ...
                if (i >= waits.length || !retryable(e) || Date.now() - t0 > 8000)
                    throw e;
     ...
     */
    async function exchange(provider, call) {
        try {
    ...
    async function appleNative() {
        const a = env.apple();
        if (!a)
            throw new AuthError('failed', 'apple_unavailable'); // 이 빌드엔 애플 네이티브가 없다
        let c;
        try {
            ...
            if ((0, exports.isCancel)(errText(e)))
                throw new AuthError('cancelled', 'apple');
            throw e;
        }
    ...
    async function exchangeTicket(ticket, nonce) {
        const w = web;
        const t0 = Date.now();
        let tries = 0;
        try {
    ...
    /** 브라우저 창 → 티켓 → 세션(2.0) */
    async function webLogin(provider, alive) {
        if (!web)
            throw new AuthError('failed', provider + '_unavailable');
        // 잊힌 실행(SDK 가 실패해 넘어온 것)은 연결 코드 요청 같은 서버 호출도 하지 않는다
        if (!alive())
            throw abandoned();
        // 이 제공자로 시작한 흐름에 늦게 온 후보가 있다 — 창을 다시 열지 않고 그것부터
        if (flowAlive() && flow && flow.provider === provider && freshCandidates().length > 0)
            return tryCandidates(flow, alive);
        const B = env.browser();
        if (!B?.openAuthSessionAsync)
            throw new AuthError('failed', 'open:no_browser');
        let link;
        try {
            /*
            ...
            if (deps.guest?.active()) {
                ...
                if (!link)
                    throw new AuthError('failed', 'open:no_guest_link');
            }
        }
        catch (e) {
        ...
        if (!alive())
            throw abandoned();
        const f = { provider, nonce, at: Date.now(), open: true };
        flow = f;
        const same = () => flow === f && f.open;
        let res;
        try {
    ...
    async function run(provider, alive) {
         ...
         */
        if (web && flowAlive() && flow && flow.provider === provider && freshCandidates().length > 0)
        ...
        if (provider === 'apple') {
            ...
            if (env.os() === 'ios')
                return appleNative();
            if (web && list.includes('apple_web'))
                return webLogin('apple', alive);
