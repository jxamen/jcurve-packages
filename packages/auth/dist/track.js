        ...
        ga: () => {
            ...
            try {
                ...
                if (RN?.Platform?.OS !== 'web'
                    ...
                    cached = require('@react-native-firebase/analytics');
                }
            }
            catch {
                cached = null;
...
function standardEvent(name, params) {
    ...
    if (name === 'signup_done')
        return ['sign_up', method];
    if (name === 'login_done')
        return ['login', method];
    if (name === 'onboarding_view' && params?.step === 1)
        return ['tutorial_begin', undefined];
    if (name === 'onboarding_done')
        return ['tutorial_complete', undefined];
    return null;
}
/**
...
function propOf(params) {
    ...
    const c = (0, funnel_1.looksPersonal)(c0) ? '' : c0;
...
function clean(params) {
    ...
    for (const [k, v] of Object.entries(params)) {
        if (v == null || PERSONAL_KEY.test(k))
            continue;
        // 숫자 값도 본다 — 회원번호·전화번호가 숫자로 들어올 수 있다
        if (typeof v !== 'boolean' && (0, funnel_1.looksPersonal)(v))
            continue;
        out[k] = v;
    }
    return out;
 ...
 */
function createTrack(opts = {}, env = defaultEnv()) {
    return (name, params) => {
        // 서버 퍼널은 네이티브 모듈과 무관하다 — GA4 가 없어도 남긴다
        try {
            opts.funnel?.event(name, propOf(params));
        }
        catch { /* 계측이 앱을 막지 않는다 */ }
        try {
            const m = env.ga();
            ...
            if (std)
                void Promise.resolve(m.logEvent(a, std[0], std[1])).catch(() => undefined);
        }
        catch { /* 계측이 앱을 막지 않는다 */ }
    };
}
