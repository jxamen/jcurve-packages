...
| 이름 | 무엇 | 화면 |
|---|---|---|
| `@jcurve/auth` | 소셜 로그인 + 가입 퍼널 기록 — 카카오·구글·애플 SDK, 안 되면 서버 웹 로그인 (2.1.1) | 없음 |
| `@jcurve/apphub` | 앱 모아보기 — 계열 앱 목록 시트 | 시트만 공용, 입구 버튼은 앱마다 |
| `@jcurve/phone` | 휴대폰 번호 인증 — 서버 호출·번호 검증·오류 문구 (1.2) | 없음 |
| `@jcurve/updates` | OTA — 받은 새 버전을 로그인과 겹치지 않는 순간에 적용 (꼬꼬농장 방식, 2.4.1 — 앞으로 올 때 받기 선택, 로그인 중엔 안 받음) | 없음 |
...
| `@jcurve/ads` | AdMob 보상형 광고 — 안전하게 띄우고 끝까지 봤는지 알려 준다(꼬꼬농장 방식, 1.3 — 미리 받지 않음 · ATT 는 켤 때) | 없음 (대기·목업 화면은 앱마다) |
